import React, { useEffect, useState, useMemo, useCallback, useRef } from "react";
import UserPagination from "../../../components/common/UserPagination";
import { MdFavorite } from "react-icons/md";
import { TfiLocationPin } from "react-icons/tfi";
import { toast } from "react-toastify";
import axios from "axios";
import Loader from "components/common/Loader";
import { getFullLocation } from "utils/locationHelper";
import { useNavigate } from "react-router-dom";
import { useAuth } from "context/AuthContext";

export default function SavedAds() {
  const apiUrl = process.env.REACT_APP_API_URL;
  const { token } = useAuth();
  const navigate = useNavigate();

  const [ads, setAds] = useState([]);
  const [filter, setFilter] = useState("All");
  const [search, setSearch] = useState("");
  const [debouncedSearch, setDebouncedSearch] = useState("");
  const [currentPage, setCurrentPage] = useState(1);
  const [loading, setLoading] = useState(false);

  const adsPerPage = 9;

  /** Debounce search */
  useEffect(() => {
    const handler = setTimeout(() => setDebouncedSearch(search), 250);
    return () => clearTimeout(handler);
  }, [search]);

  /** Fetch saved */
  const fetchSavedPosts = useCallback(async () => {
    setLoading(true);
    try {
      const res = await axios.get(`${apiUrl}save/list`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      setAds(res.data.data || []);
    } catch (err) {
      console.error(err);
      toast.error("Failed to fetch saved posts");
    } finally {
      setLoading(false);
    }
  }, [apiUrl, token]);

  useEffect(() => {
    fetchSavedPosts();
  }, [fetchSavedPosts]);

  const handleAdClick = useCallback(
    (ad) => {
      const postId = ad.postId;
      switch (ad.postCategory) {
        case "Space":
          navigate(`/spaces/${postId}`);
          break;
        case "Spacewanted":
          navigate(`/place-wanted/${postId}`);
          break;
        case "Teamup":
          navigate(`/team-up/${postId}`);
          break;
        default:
          toast.error("Unknown post category");
      }
    },
    [navigate]
  );

  const handleRemoveSaved = useCallback(
    async (ad) => {
      try {
        const res = await axios.post(
          `${apiUrl}save`,
          {
            postId: ad.postId,
            postCategory: ad.postCategory,
            listingPage: ad.postCategory,
          },
          { headers: { Authorization: `Bearer ${token}` } }
        );

        toast.success(res.data.message || "Removed from saved");

        // local remove without refetch
        setAds((prev) => prev.filter((s) => s._id !== ad._id));
      } catch (err) {
        console.error("Error removing saved ad:", err);
        toast.error("Failed to remove saved ad");
      }
    },
    [apiUrl, token]
  );

  /** Filter + search + pagination = memoized */
  const filteredAds = useMemo(() => {
    if (!ads) return [];
    return ads.filter(
      (ad) =>
        (filter === "All" || ad.postCategory === filter) &&
        (debouncedSearch === "" ||
          ad.snapshot?.title?.toLowerCase().includes(debouncedSearch.toLowerCase()))
    );
  }, [ads, filter, debouncedSearch]);

  const totalPages = Math.ceil(filteredAds.length / adsPerPage);

  const paginatedAds = useMemo(() => {
    const start = (currentPage - 1) * adsPerPage;
    return filteredAds.slice(start, start + adsPerPage);
  }, [filteredAds, currentPage]);

  const filters = [
    { key: "All", label: "All Posts" },
    { key: "Space", label: "Spaces" },
    { key: "Spacewanted", label: "Space Wanted" },
    { key: "Teamup", label: "Team Up" },
  ];

  return (
    <div className="container user-layout mt-5 mb-8">
      {/* Filters + Search */}
      <div className="flex flex-col lg:flex-row md:justify-between md:items-center gap-3 mb-5">
        <div className="flex flex-wrap gap-2 text-[14px] font-semibold w-full md:max-w-xl text-black">
          {filters.map(({ key, label }) => (
            <button
              key={key}
              onClick={() => {
                setFilter(key);
                setCurrentPage(1);
              }}
              className={`flex-1 min-w-[120px] py-2 rounded-[2px] border-[1px] border-[#AACCEE] ${
                filter === key ? "bg-[#565ABF] text-white" : "bg-white"
              }`}
            >
              {label}
            </button>
          ))}
        </div>

        <div className="w-full md:w-auto">
          <input
            type="text"
            placeholder="Search Ads"
            value={search}
            onChange={(e) => {
              setSearch(e.target.value);
              setCurrentPage(1);
            }}
            className="w-full md:w-[250px] border-[1px] border-[#D1D5DB] rounded-[5px] py-2 px-4"
          />
        </div>
      </div>

      {/* Listing */}
      {loading ? (
        <Loader />
      ) : paginatedAds.length > 0 ? (
        <div className="flex flex-wrap gap-3 justify-start">
          {paginatedAds.map((ad) => {
            const post = ad.snapshot;
            const city = post.city === "NA" ? "" : post.city;
            const locationString = getFullLocation(city, post.state, post.country);

            return (
              <SavedCard
                key={ad._id}
                ad={ad}
                post={post}
                locationString={locationString}
                onRemove={handleRemoveSaved}
                onClick={handleAdClick}
              />
            );
          })}
        </div>
      ) : (
        <p className="text-gray-500">No saved ads found.</p>
      )}

      {/* Pagination */}
      <div className="mt-6 flex justify-center md:justify-end">
        <UserPagination
          currentPage={currentPage}
          totalPages={totalPages}
          onPageChange={setCurrentPage}
        />
      </div>
    </div>
  );
}

/** Card memoized + lazy loading optimized */
const SavedCard = React.memo(function SavedCard({ ad, post, locationString, onRemove, onClick }) {
  const img = post?.photo?.[0]?.url || post?.photo;
  const [loaded, setLoaded] = useState(false);

  return (
    <div
      className={`w-full sm:w-[48%] lg:w-[31.8%] border-[1px] border-[#D7D7D7] rounded-[12px] shadow-lg overflow-hidden flex flex-col p-4 relative ${
        ad.isDeleted ? "opacity-60" : ""
      }`}
    >
      {ad.isDeleted ? (
        <div className="flex flex-col items-center justify-center h-[300px] text-center">
          <h3 className="text-lg font-semibold text-gray-600 mb-2">Post Deleted</h3>
          <button
            onClick={(e) => {
              e.stopPropagation();
              onRemove(ad);
            }}
            className="bg-[#FF015E] text-white px-4 py-2 rounded-md text-sm"
          >
            Remove Saved
          </button>
        </div>
      ) : (
        <>
          <div className="relative">
            {!loaded && (
              <div className="animate-pulse bg-gray-200 absolute inset-0 rounded-[10px]" />
            )}

            <img
              src={img}
              alt={post?.title}
              loading="lazy"
              decoding="async"
              onLoad={() => setLoaded(true)}
              onClick={() => onClick(ad)}
              className={`cursor-pointer w-full h-[220px] sm:h-[260px] lg:h-[280px] object-cover rounded-[10px] transition-opacity duration-300 ${
                loaded ? "opacity-100" : "opacity-0"
              }`}
            />

            <span
              onClick={() => onRemove(ad)}
              className="cursor-pointer absolute top-4 right-4 bg-[#FF015E] text-white text-[13px] sm:text-[15px] font-semibold px-[12px] py-2 rounded-[4px] flex items-center gap-1 z-10"
            >
              <MdFavorite size={18} /> Saved
            </span>
          </div>

          <div className="flex flex-col flex-grow text-black p-1 mt-2">
            <h3
              onClick={() => onClick(ad)}
              className="cursor-pointer font-semibold text-[16px] sm:text-[18px] text-black"
            >
              {post?.title}
            </h3>

            <p className="text-[15px] sm:text-[18px] flex items-center gap-1">
              <TfiLocationPin /> {locationString}
            </p>

            <p className="font-semibold text-[15px] sm:text-[18px]">
              ₹{post?.budget} / {post?.budgetType}
            </p>

            <p className="text-[15px] sm:text-[18px] leading-[1.7rem] line-clamp-2">
              {post?.description}
            </p>
          </div>
        </>
      )}
    </div>
  );
});
